  ...
  if (searchTerm && hiddenColumns.length > 0) {
    paginatedData.forEach((rowObject: IRowData) => {
      hiddenColumns.forEach((hiddenKey) => {
        ...
        if (isValidKey(hiddenKey, rowObject) && checkIfCellContainsSearchTerm(rowObject[hiddenKey], searchTerm)) {
        ...
        // if hidden key is a key associated with percentages, check those keys as well
          ...
          const keyPercent = `${hiddenKey}Percent`;
          if (isValidKey(keyPercent, rowObject) && checkIfCellContainsSearchTerm(rowObject[keyPercent], searchTerm)) {
            foundSearchTermInHiddenColumn = true;
          }
        }
      });
